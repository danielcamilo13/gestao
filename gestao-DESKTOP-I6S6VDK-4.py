...
print('########################################')
print(' Ultima atualizacao 28 de Julho 2017')
print(' exibindo PID\'s de processos')
print('########################################')
    ...
    try:
        ...
        for cell in cells:
            ...
            for node in nodes:
                ...
                serverAttrs = AdminControl.queryNames('WebSphere:type=Server,cell=%s,node=%s,*'%(cellName,nodeName)).split()
                ...
                for srv in serverAttrs:
                    ...
                    arqproc.write('ssh -t %s \'processo: %s %s\' %s\n'%(nodeHostname,sname,pid,ptype))
        print('gerado PID.sh, arquivo com com os processos filhos e seus respectivos pids.')
        arqproc.close()
    except:
...
def instalar(app1,path,cluster):
    apps = [app for app in AdminApp.list().split() if app == app1][0]
    if len(apps)<1:
        print('Aplicacao %s nao localizada\n Prosseguindo com a instalacao '%apps)
...
def statusapps():
    #Status das aplicacao
    print('verificando o status da aplicacao ....')
    arqsys.write('#'*50+'\n')
    cellName = [AdminConfig.showAttribute(cell,'name') for cell in AdminConfig.list('Cell').split()]
    if len(cellName)==1:
        cellID = AdminConfig.list('Cell').split()[0]
        arqsys.write('Nome da Celula: %s'%cellName[0]+'\n')
        ...
        for nodeName in nodeNames:
            ...
            serverNames = [AdminControl.getAttribute(server,'name') for server in AdminControl.queryNames('type=Server,cell=%s,node=%s,*'%(cellName[0],str(nodeName))).split()]
            for serverName in serverNames:
                arqsys.write('Nome do servidor: %s\n'%serverName)
                objNames = [AdminControl.queryNames('type=Application,name='+app+',*').split() for app in AdminConfig.list('Websphere:cell='+cellName[0]+',node='+str(nodeName)+',server='+str(serverName)+'').split()]
                for objName in ObjNames:
                    arqsys.write('Status das aplicacoes %s\n'%objName)
    ...
    # for cell in cells:
        ...
        # for node in nodes:
            ...
            # for server in servers:
                ...
                # apps = AdminApp.list('Websphere:cell='+cellName+',node='+nodeName+',server='+serverName).split()
                # for app in apps:
                    # arqsys.write('Pacote: %s'%app+'\n')
                    # objNames = AdminControl.queryNames('type=Application,name=%s,*'%app).split()
                    # arqsys.write('Status das aplicacoes %s\n'%objNames)
                    # if len(objNames)!=0:
                        # ct = 0
                        # for objName in objNames:
...
# def statusapps():
    # #Status das aplicacao
    # print('verificando o status da aplicacao ....')
    # cellNameCL = [AdminConfig.showAttribute(cell,'name') for cell in AdminConfig.list('Cell').split()]
    # print('Compreensao de listas %s'%cellNameCL)
    # cells = [AdminConfig.list('Cell').split()]
    # for cell in cells:
        # cellName = AdminConfig.showAttribute(cell,'name')
        # nodes = AdminConfig.list('Node',cell).split()
            ...
            for server in servers:
                ...
                for app in apps:
                    if app == app1:
                        ...
                        print('Nome do Servidor %s'%serverName)
                        ...
                        print('Dados do objeto localizado %s'%objName)
                        if objName == '':
...
def modifica_resource(path3,jar,camada):    
    ...
    arqmod.write('            domapp=parse(\'%s/applicationSettings.xml\')\n'%(path3))  
    ...
    arqmod.write('            arqvar=open(\'%s/variableCfgSelector.xml\',\'r+\')\n'%(path3))  
    arqmod.write('            arqvard=open(\'%s/novo_variableCfgSelector.xml\',\'w\')\n'%(path3))  
    ...
    arqmod.write('            domapp.writexml(open(\'%s/novo_applicationSettings.xml\',\'w\'),addindent=\'\',newl=\'\')\n'%(path3))
    arqmod.write('            domapp.unlink()\n')
    arqmod.write('os.remove(\'%s/applicationSettings.xml\')\n'%(path3))
    arqmod.write('os.rename(\'%s/novo_applicationSettings.xml\',\'%s/applicationSettings.xml\')\n'%(path3,path3))
    arqmod.write('os.remove(\'%s/variableCfgSelector.xml\')\n'%(path3))
    arqmod.write('os.rename(\'%s/novo_variableCfgSelector.xml\',\'%s/variableCfgSelector.xml\')\n'%(path3,path3))
    arqmod.write('os.system(\'chmod -R 775 %s\')\n'%(path3))
    arqmod.close()
    os.system('chmod 775 extrair.py')
    os.system('python extrair.py') 
...
try:
    ...
    print('execucao da rotina gestao.py finalizada. Veja o arquivo SystemExit para retorno')
    arqsys.close()
