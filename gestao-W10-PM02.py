...
print('########################################')
print(' Ultima atualizacao 01 de Setembro 2017')
print(' correcao do start, stop e restart de aplicacoes')
print('########################################')
...
def props():
    ...
    for cell in AdminConfig.list('Cell').split():
        for node in AdminConfig.list('Node',cell):
            print(AdminConfig.showAttribute(node,'hostName'))
            arqproc.write('ssh -t %s \'cat /ArquitecturaE-business/Lib/LibRigel/Conf_Aplicaciones/%s \'\n'%(AdminConfig.showAttribute(node,'hostName'),ens))
    # arqproc.close()
    # try:
        # print('argumento informado %s'%arg)
        # arqproc = open('pids.sh','w')
